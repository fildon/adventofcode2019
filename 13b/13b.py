...
class IntCodeComputer:
    ...
    def process_command_at_pointer(self):
        ...
        raw_args = [self.int_code.read(self.command_pointer + i + 1)
                    for i in range(len(write_args))] # Used only for the debug statement below
...
class Frame:
    ...
    def parse_input(self, int_code_output):
        while len(int_code_output) >= 3:
            xy = (int_code_output.pop(0), int_code_output.pop(0))
            value = int_code_output.pop(0)
            if xy == (-1, 0):
                self.score = value
            else:
                self.set_colour(xy, value)
    ...
    def draw_to_console(self):
        [xmin, ymin, xmax, ymax] = self.x_y_min_max()
